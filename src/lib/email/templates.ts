...
const formatDate = (value: Date) =>
  value.toLocaleString('en-US', { timeZone: 'UTC', dateStyle: 'medium', timeStyle: 'short' });
...
export const emailTemplates: { [K in EmailEvent]: (data: TemplateDataMap[K]) => EmailPayload } = {
  user_registered_contractor: createTemplate(
    ...
    ({ name }) => `
      ...
      <p>The help drawer in your dashboard has step-by-step checklists.</p>
    `,
  ),
  ...
  invite_sent: createTemplate(
    ...
    ({ inviterName, jobTitle, acceptUrl }) => `
      ...
      <p><a href="${acceptUrl}">Accept the invitation</a> to review milestones and escrow terms.</p>
    `,
  ),
  invite_accepted: createTemplate(
    ...
    ({ inviteeEmail, jobTitle }) => `
      ...
      <p>Log in to configure funding and ready the first milestone.</p>
    `,
  ),
  ...
  milestone_submitted: createTemplate(
    ...
    ({ jobTitle, milestoneTitle }) => `
      ...
      <p>Review the evidence and respond within the review window.</p>
    `,
  ),
  milestone_reminder_24h: createTemplate(
    'Reminder: milestone auto-approval in 24 hours',
    ({ jobTitle, milestoneTitle, dueAt }) => `
      <p>"${milestoneTitle}" for "${jobTitle}" will auto-approve within 24 hours (by ${formatDate(dueAt)}).</p>
      <p>Respond in Conforma if additional work is needed.</p>
    `,
  ),
  ...
  milestone_auto_approved: createTemplate(
    ...
    ({ jobTitle, milestoneTitle }) => `
      ...
      <p>Ensure future evidence is submitted promptly to avoid auto-release surprises.</p>
    `,
  ),
  dispute_opened: createTemplate(
    ...
    ({ jobTitle, milestoneTitle, reason }) => `
      ...
      <p>Upload evidence and respond to keep the project moving.</p>
    `,
  ),
  ...
  document_approved: createTemplate(
    'Document approved',
    ({ type }) => `
      <p>Your ${type.toLowerCase()} document is verified.</p>
      <p>Verified badges update automatically in your profile and search placements.</p>
    `,
  ),
  document_rejected: createTemplate(
    ...
    ({ type, reason }) => `
      ...
      <p>Upload a corrected version to restore your verified badge.</p>
    `,
  ),
  document_needs_review: createTemplate(
    'Document needs manual review',
    ({ type, reason }) => `
      <p>Your ${type.toLowerCase()} document needs additional review.</p>
      ${reason ? `<p>Notes: ${reason}</p>` : ''}
      <p>We will notify you once a human reviewer completes the check.</p>
    `,
  ),
  document_expiring_soon: createTemplate(
    'Document expiring soon',
    ({ type, effectiveTo }) => `
      <p>Your ${type.toLowerCase()} document expires on ${formatDate(effectiveTo)}.</p>
      <p>Upload an updated document to keep your verified badge active.</p>
    `,
  ),
  payout_initiated: createTemplate(
    'Payout initiated',
    ({ amount, currency }) => `
      <p>Your payout is in progress.</p>
  ...
  risk_review_required: createTemplate(
    ...
    ({ jobTitle, reasons, score }) => `
      <p>Job "${jobTitle}" requires manual review before releasing funds.</p>
